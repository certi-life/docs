...
test('credential scanner는 공개 Markdown의 UUID·전화번호·내부 주소를 차단한다', () => {
  ...
  for (const body of [
    ...
    '내부 주소 10‧0‧0‧1',
    '내부 주소 10·0·0·1',
    `연락처 010${String.fromCodePoint(0xfe0f)}-1234-5678`,
    `내부 주소 10${String.fromCodePoint(0xfe0f)}.0.0.1`,
    `연락처 02/${String.fromCodePoint(0x338)}1234/${String.fromCodePoint(0x338)}5678`,
    '내부 주소 fd00∶∶1',
    '연락처 010֊1234֊5678',
    '연락처 010᠆1234᠆5678',
    '연락처 02╱1234╱5678',
    ...
    String.raw`공개 문서 https://docs.certi.life/a\<010-1234-5678`,
    '내부 주소 fd00::1',
    '[내부 주소](http://[fd00::1]/)',
    '[IPv4 매핑 주소](http://[::ffff:a00:1]/)',
    '[IPv6 주소](http://[2001:db8::1]/)',
  ]) {
    assert.throws(() => createCleanMarkdownArtifacts(document(body)), /private or credential-like content detected/, body);
  }
});
