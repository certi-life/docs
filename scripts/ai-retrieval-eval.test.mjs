...
test('validateFixtures는 중복·비공개 식별자·불완전 Top3를 fail-closed로 거부한다', () => {
  ...
  assert.throws(() => validateFixtures([{...base, question: '주소 1.2.3.4 확인'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'notversion 1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'notversion v1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '제품version 1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '제품\u0301version 1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '제품\u0301version v1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '제품\u203Fversion 1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version\n1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version\nv1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '내부 기능 버전 10.0.0.7 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  ...
  assert.throws(() => validateFixtures([{...base, question: '연락처 011-123-4567 확인'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  ...
  assert.throws(() => validateFixtures([{...base, question: '공개 기능 버전 1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.doesNotThrow(() => validateFixtures([{...base, question: '공개 기능 버전 v1.2.3.4 안내'}], new Set(base.expectedTop3), {expectedCount: 1}));
  assert.doesNotThrow(() => validateFixtures([{...base, question: 'public feature version v10.999.999.999'}], new Set(base.expectedTop3), {expectedCount: 1}));
  assert.doesNotThrow(() => validateFixtures([{...base, question: 'public feature version v1.2.3.45'}], new Set(base.expectedTop3), {expectedCount: 1}));
  assert.doesNotThrow(() => validateFixtures([{...base, question: 'public feature version v1.2.3.4.'}], new Set(base.expectedTop3), {expectedCount: 1}));
  assert.throws(() => validateFixtures([{...base, question: 'public feature version v1.2.3.4.5'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version v1.2.3.4\u0301'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version v1.2.3.4\u203F'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version v1.2.3.4_suffix'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: 'version v1.2.3.4x'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '연락처 +82 (10) 1234 5678 확인'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
  assert.throws(() => validateFixtures([{...base, question: '연락처 +82 (70) 1234.5678 확인'}], new Set(base.expectedTop3), {expectedCount: 1}), /non-public identifier/);
